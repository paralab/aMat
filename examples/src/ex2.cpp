 ...
 * @brief Partition of elements in y direction: owned elements in y direction ~ Ney/(number of
 * ranks)
...
#define MAX_BLOCKS_PER_ELEMENT (1u << AMAT_MAX_CRACK_LEVEL)
...
int main(int argc, char* argv[])
{
    ...
    {
        globalMap[eid] = new unsigned long[MAX_BLOCKS_PER_ELEMENT * NNODE_PER_ELEM];
    }
    ...
    for (unsigned int i = 0; i < constrainedDofs.size(); i++){
        printf("rank %d constraint %d, global ID %d, prescribed value
    ...
    for (unsigned int eid = 0; eid < nelem; eid++)
    {
        ...
        if (traction)
        {
            // ad-hoc setting force vector due to uniform traction applied on face 2-3 of the
            // element compute area of the top face
