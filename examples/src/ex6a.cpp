 ...
 * @brief This is same as ex1 but generate local map first, then create global map based on local map
 * @brief to prepare for next step is to enrich by update local map
 * 
...
int main( int argc, char *argv[] ) {
    ...
    for (unsigned int eid = 0; eid < nelem; eid++) {
        localDofMap[eid] = new unsigned int[MAX_BLOCKS_PER_ELEMENT * NNODE_PER_ELEM * NDOF_PER_NODE];
    }
    for (unsigned int eid = 0; eid < nelem; eid++) {
    ...
    for (unsigned int eid = 0; eid < nelem; eid++){
        ...
        // suppose eid = 21 is a cracked element, allocate more blocks
        if (eid == 21){
            for (unsigned int i = 1; i < MAX_BLOCKS_PER_ELEMENT; i++){
                kee[i] = new EigenMat;
                fee[i] = new EigenVec;
        ...
        non_zero_block_i[eid][0] = 0;
        non_zero_block_j[eid][0] = 0;
        if (matType == 0){
            petsc_time.start();
