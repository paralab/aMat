...
int main(int argc, char* argv[])
{
    ...
    for (unsigned int eid = 0; eid < nelem; eid++)
    {
        for (unsigned int nid = 0; nid < NNODE_PER_ELEM; nid++)
        {
            ...
            // node at corner diagonally opposite with origin --> fix in x direction to prevent
            // rotation in z
            if ((fabs(x - Lx) < zero_number) && (fabs(y - Ly) < zero_number) &&
                (fabs(z) < zero_number))
            {
                bound_dofs[eid][nid * NDOF_PER_NODE]   = 1;
                bound_values[eid][nid * NDOF_PER_NODE] = 0.0;
    ...
    for (unsigned int eid = 0; eid < nelem; eid++)
    {
        ...
        if (traction)
        {
            ...
            // get nodal traction of face where traction is applied (uniform traction t0 = 1.0
    ...
    unsigned int nz_i[MAX_BLOCKS_PER_ELEMENT], nz_j[MAX_BLOCKS_PER_ELEMENT];
    ...
    for (unsigned int eid = 0; eid < nelem; eid++)
    {
        ...
        if (matType == 0)
        {
            ...
            stMatBased->set_element_matrix(eid, nz_i, nz_j, (const EigenMat**)kee, 1u);
        }
        else
        {
            // stMatFree->set_element_matrix(eid, kee[0], 0, 0, 1);
            stMatFree->set_element_matrix(eid, nz_i, nz_j, (const EigenMat**)kee, 1u);
        }
