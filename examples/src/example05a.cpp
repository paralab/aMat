/**
 * @file example05a.cpp, example05a.hpp
 * 
 * @author Hari Sundar   hsundar@gmail.com
 * @author Han Duc Tran  hantran@cs.utah.edu
 * 
 * @brief Solving 3D Poisson equation by FEM, in parallel, using aMat with quadratic 10-node tetrahedra elements (mesh is generated by Gmsh)
 * @brief emesh file generated by Gmsh: ex5_quad_4p.msh (272841 nodes, partition to run with 4 processes)
 *                                      ex5_quad_8p.msh (272841 nodes, partition to run with 8 processes)
 * @brief (this example was ex6 or ex6a in in aMat_for_paper/)
 ...
 * @brief Exact solution: phi = (L^2 / (12.0 * M_PI * M_PI)) * sin(2 * M_PI * x/L) * sin(2 * M_PI * y/L) * sin(2 * M_PI * z/L);
 ...
 */
#include "example05a.hpp"

// global variable, accessed by any function
AppData example05aAppData;
...
void computeElemMat(unsigned int eid, double* ke, double* xe) {
   GmshMesh * p_mesh = example05aAppData.p_mesh;
   ...
   if (nnodes == 10) {
      ke_tet10(ke, xe, example05aAppData.intData->Pts_n_Wts, example05aAppData.NGT);
   }
   else {
...
void usage() {
    std::cout << "\n";
    std::cout << "Usage:\n";
    std::cout << "./example05 <method> <BC method> <n Streams> <meshfile> <outFile>\n";
    std::cout << "\n";
    exit(0);
}
...
int main(int argc, char* argv[]) {
   ...
   const unsigned int NDOF_PER_NODE = 1;        // number of dofs per node, depends on problem
   const double L = 1.0;                        // size of the cube
   const double ZERO_NUMBER = 1E-12;
   const unsigned int NGT = 2;                  // number of integration points in 1D dimension
   integration<double> intData(NGT);            // object holding quadrature points and weights
   ...
   const char* outputFile = argv[5]; // output file name
   ...
   unsigned int ** localMap = mesh.get_localMap();

   // ========================================================
   // set parameters to global variable so that matrix-free element-mat function can use them
   example05aAppData.NGT = NGT;
   example05aAppData.intData = &intData;
   example05aAppData.p_mesh = &mesh;
   
   // provide Gmsh mesh to aMat ========================================================
   par::Maps<double, unsigned long, unsigned int> meshMaps(comm);
   meshMaps.set_map(nelem_owned, localDofMap, nDofsPerElem, nLocalDofs,
                  local2GlobalDofMap, startGlobalDof, endGlobalDof, nDofsTotal);
   ...
   // determine max n nodes per element over the mesh
   int MAX_NNODE_PER_ELEM = 0;
   for (unsigned int e = 0; e < nelem_owned; e++) {
      if (nNodesPerElem[e] > MAX_NNODE_PER_ELEM) MAX_NNODE_PER_ELEM = nNodesPerElem[e];
   }
   const int MAX_NDOFS_PER_ELEM = MAX_NNODE_PER_ELEM * NDOF_PER_NODE;

   EigenMat ke;
   EigenVec fe;

   double * xe = new double [MAX_NNODE_PER_ELEM * NDIM];
   double * be = new double [MAX_NNODE_PER_ELEM * NDOF_PER_NODE];
   ...
   for (unsigned int eid = 0; eid < nelem_owned; eid++) {
      ...
      // resize the element matrix and vector (in Eigen, it does not do anything if the actual matrix size does not change)
      ke.resize(nnodes, nnodes);
      fe.resize(nnodes, 1);

      // compute element stiffness matrix
      setup_time.start();
      elem_compute_time.start();
      ke_tet10(ke, xe, intData.Pts_n_Wts, NGT);
      ...
      fe_tet10(fe, xe, be, intData.Pts_n_Wts, NGT);
