...
Mat& aMatFree<DT, GI, LI>::get_matrix()
{
    ...
    // create matrix shell
    MatCreateShell(m_comm, m_uiNumDofs, m_uiNumDofs, PETSC_DETERMINE, PETSC_DETERMINE, ctx, &m_pMat);
    ...
    MatShellSetOperation(m_pMat, MATOP_GET_DIAGONAL, (void (*)(void))aMat_matgetdiagonal<DT, GI, LI>);
            ...
            for (LI block_j = 0; block_j < blocks_dim; block_j++) {
                ...
                if (m_epMat[eid][index] != nullptr) {
                    ...
                    for (LI r = 0; r < num_dofs_per_block; r++) {
                        ...
                        rank_r = m_maps.globalId_2_rank(glo_RowId); // globalId_2_rank() is moved to Maps class
                        ...
                        for (LI c = 0; c < num_dofs_per_block; c++) {
                            ...
                            rank_c = m_maps.globalId_2_rank(glo_ColId); // globalId_2_rank() is moved to Maps class
...
#pragma omp simd aligned(x, veLocal \
                         : ALIGNMENT) safelen(512)
...
PetscErrorCode aMatFree<DT, GI, LI>::MatGetDiagonalBlock_mf(Mat A, Mat* a)
{
    ...
    MatCreateSeqAIJ(PETSC_COMM_SELF, m_uiNumDofs, m_uiNumDofs, NNZ, PETSC_NULL, a); // todo: 27 only good for dofs_per_node = 1
...
// accumulate Kfc[r,c]*Uc[c]
                                        ...
                                        temp += m_epMat[eid][block_index][(c * num_dofs_per_block) + r] * m_dtPresValMap[eid][block_j * num_dofs_per_block + c];
#elif VECTORIZED_OPENMP_ALIGNED
                                        // block m_epMat[eid][block_index] is stored in column-major with nPads appended to columns
                                        temp += m_epMat[eid][block_index][c * (num_dofs_per_block + nPads) + r] * m_dtPresValMap[eid][block_j * num_dofs_per_block + c];
#else
                                        // block m_epMat[eid][block_index] is stored in row-major
                                        temp += m_epMat[eid][block_index][(r * num_dofs_per_block) + c] * m_dtPresValMap[eid][block_j * num_dofs_per_block + c];
#endif
